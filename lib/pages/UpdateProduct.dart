...
class UpdateProduct extends StatefulWidget {
  final String id; // this is the id of the product
  final DocumentSnapshot product;

  const UpdateProduct({super.key, required this.id, required this.product});
  @override
...
class _UpdateProductState extends State<UpdateProduct> {
  ...
  String productImageUpdated = ''; // seve the last image to show it
  // loanding
  ...
  void initState() {
    ...
    super.initState();
    // we add the data that we recive from the oder page
    setState(() {
      productNameController.text = productData['name'];
      productCantidadController.text = productData['quantity'].toString();
      productPrecioController.text = productData['price'].toString();
      productDescriptionController.text = productData['description'];
      productImageUpdated = productData['imageUrl'];
    });
    _getCategories().then((result) {
      setState(() {
        categories = result;
        categoriesDropDown = getCategoriesDropDown();
        _currenteCategory = productData['category'];
      });
    });
    ...
    _getBrands().then((resultBrands) {
      setState(() {
        ...
        _currenteBrand = productData['brand'];
      });
    });
  }
  ...
  List<DropdownMenuItem<String>> getCategoriesDropDown() {
    ...
    for (int i = 0; i < categories.length; i++) {
      // if(categories[i]['categoryName'] == widget.product['category']){
      //   _currenteCategory = widget.product['category'];
      // }
      items.insert(
        0,
        DropdownMenuItem(
                        ...
                        child: Padding(
                          ...
                          child: Container(
                            ...
                            child: _pickedImage == null
                                ? Image.network(productImageUpdated)
                                : kIsWeb
                                    ? ClipRRect(
                                        borderRadius: BorderRadius.circular(12),
  ...
  void updateProduct() async {
    ...
    try {
      ...
      if (_pickedImage != null) {
        // if "_pickedImage" is not null that means that I chose a new image
        // saving and getting the URL
        ...
        // if "_pickedImage" is null that means I did not choose any image
        dowloandUrl = productImageUpdated;
      }

      // print(widget.id);
      // print(productNameController.text.trim());
      // print(productPrecioController.text.trim());
      // print(productCantidadController.text.trim());
      // print(productDescriptionController.text.trim());
      // print(_currenteBrand);
      // print(_currenteCategory);
      // print(dowloandUrl);
